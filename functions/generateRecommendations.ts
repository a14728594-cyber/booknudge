...
2. 「description（本の概要）」の内容もユーザーの悩み・状況と合致するか確認する。
3. 「outcomes（読んだ後こうなれる）」がユーザーの悩みの解決につながるかを確認する。
4. 「one_liner」「tags」も参考にして総合的に判断する。
5. 「not_for」にユーザーの状況が当てはまる本は除外する。
6. ジャンル「${genre}」に無関係な本は優先度を下げる。
7. 文字列の完全一致ではなく、意味・文脈の類似度で判断する。
