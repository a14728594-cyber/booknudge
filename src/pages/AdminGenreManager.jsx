...
import { Plus, Trash2, Check, X, ChevronDown, ChevronRight, ArrowLeft } from 'lucide-react';
...
export default function AdminGenreManager() {
    ...
    const [addingProblem, setAddingProblem] = useState(null); // genre_id
    ...
    const deleteGenre = async (genre) => {
        if (!confirm(`「${genre.name}」を削除しますか？（紐づく悩みカテゴリも削除されます）`)) return;
    ...
    const updateProblem = async (problem) => {
        ...
        setProblems(prev => ({
            ...prev,
            [problem.genre_id]: prev[problem.genre_id].map(p => p.id === problem.id ? { ...p, name } : p)
        }));
    ...
    const deleteProblem = async (problem) => {
        ...
        setProblems(prev => ({
            ...prev,
            [problem.genre_id]: prev[problem.genre_id].filter(p => p.id !== problem.id)
        }));
    };
    ...
    const toggleProblemActive = async (problem) => {
        ...
        setProblems(prev => ({
            ...prev,
            [problem.genre_id]: prev[problem.genre_id].map(p => p.id === problem.id ? { ...p, is_active: !problem.is_active } : p)
        }));
    };
    ...
    return (
        <div className="max-w-3xl mx-auto px-6 py-8">
            <div className="flex items-center gap-3 mb-8">
                <button onClick={() => navigate(createPageUrl('AdminCaseQuiz'))} className="p-1.5 hover:bg-gray-100 rounded-lg">
                    <ArrowLeft className="w-5 h-5 text-gray-500" />
                </button>
                <h1 className="text-2xl font-bold text-gray-900">ジャンル・悩みカテゴリ管理</h1>
            </div>
                ...
                <div className="space-y-3">
                        ...
                        <div key={genre.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 px-4 py-3">
                                ...
                                {editingGenre?.id === genre.id ? (
                                    <div className="flex items-center gap-2 flex-1">
                                        <Input
                                            ...
                                            onChange={e => setEditingGenre(prev => ({ ...prev, name: e.target.value }))}
                                            onKeyDown={e => e.key === 'Enter' && updateGenre(genre)}
                                    ...
                                    <button
                                        className="flex-1 text-left font-medium text-gray-800 text-sm hover:text-indigo-600"
                                        onDoubleClick={() => setEditingGenre({ id: genre.id, name: genre.name })}
                                    >
                                        {genre.name}
                                    </button>
                                )}
                                <span
                                    ...
                                    className={`text-xs px-2 py-0.5 rounded-full cursor-pointer select-none ${genre.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}
                                >
                                ...
                                <div className="border-t border-gray-100 bg-gray-50 px-6 py-3 space-y-2">
                                    {(problems[genre.id] || []).map(prob => (
                                        <div key={prob.id} className="flex items-center gap-2 py-1">
                                            {editingProblem?.id === prob.id ? (
                                                <div className="flex items-center gap-2 flex-1">
                                                    <Input
                                                        ...
                                                        onChange={e => setEditingProblem(prev => ({ ...prev, name: e.target.value }))}
                                                        onKeyDown={e => e.key === 'Enter' && updateProblem(prob)}
                                                        className="h-7 text-sm"
                                                        autoFocus
                                                    />
                                            ...
                                            <span
                                                onClick={() => toggleProblemActive(prob)}
                                                className={`text-xs px-2 py-0.5 rounded-full cursor-pointer select-none ${prob.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}
                                            >
                                    ...
                                    {addingProblem === genre.id ? (
                                        <div className="flex items-center gap-2 mt-1">
                                            <Input
                                                ...
                                                onKeyDown={e => e.key === 'Enter' && createProblem(genre.id)}
                                                placeholder="悩みカテゴリ名"
                                                className="h-7 text-sm"
                                                autoFocus
                                            />
                                        ...
                                        <button
                                            onClick={() => setAddingProblem(genre.id)}
                                            className="text-xs text-gray-400 hover:text-indigo-600 flex items-center gap-1 mt-1"
                                        >
                    ...
                    {addingGenre ? (
                        ...
                        <button
                            ...
                            className="w-full py-3 rounded-xl border border-dashed border-gray-300 text-sm text-gray-400 hover:text-indigo-600 hover:border-indigo-400 flex items-center justify-center gap-2"
                        >
