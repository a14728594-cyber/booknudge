...
1. target_clarity: ターゲットの解像度
   - 0〜4点：「女性」「会社員」「健康志向」など属性を並べただけ。具体的な人が浮かばない
...
2. value_clarity: 価値の訴求力
   ...
   - 5〜9点：何かは伝わるが他の店でも言えることしか書いていない
   - 10〜14点：差別化につながる要素はあるが、伝わり方が弱い
   - 15〜20点：ここだけの価値が一言で刺さる言葉・仕組みとして表現されている

3. price_reasonability: 価格の納得感
   - 0〜4点：価格の記載なし、または相場から大きくズレていて根拠が不明
   - 5〜9点：価格はあるが、なぜその価格なのかの根拠・心理的価値が伝わらない
   - 10〜14点：ある程度妥当だが、価格を正当化するストーリーが弱い
   - 15〜20点：価格とターゲットの財布感覚・提供価値が見事にマッチしており納得感がある

4. differentiation: 差別化の実質
   - 0〜4点：「こだわり」「丁寧」「高品質」「本格的」のみ。これは差別化ではない
   - 5〜9点：差別化しようとしている意図はわかるが、競合でもできることしか書いていない
   - 10〜14点：一定の独自性はある。ただし模倣しやすい・継続が難しそう
   - 15〜20点：競合が簡単に真似できない仕組み・資源・ストーリーに基づいた差別化がある

5. action_strength: 行動喚起力
   - 0〜4点：「来てね」「SNS発信します」レベル。具体的な動機づけが何もない
   - 5〜9点：告知手段はあるが、「今すぐ行きたい」と思わせる仕掛けがない
   - 10〜14点：行動に繋がりそうな要素はある。しかしその強さ・確度が弱い
   - 15〜20点：ターゲットが「これは自分のことだ」と感じ、すぐ行動に移せるトリガーがある
...
【想定顧客の反応（100人中）】
この企画を実際の市場に出した場合、想定顧客100人のうち何人が入店するかを、スコア合計に連動してリアルに返してください。
enter の人数 ≈ スコア合計点（例：合計45点なら enter=45）。甘く見積もらないこと。
total_customers は常に100にしてください。

【strength_hint】
ログイン前に見せる「強みのヒント」を1文で（具体的だが詳細は隠す）。良い点がなければ正直に「惜しい部分あり」と書く。

【one_line_comment】
辛口の一言コメントを15字以内で（例：「ターゲットが全員になってる」「価格だけが取り柄」「本当に戦える企画」）。

【summary】
全体の総評を2〜3文で（ログイン後に表示）。社交辞令なし。問題点を先に、良い点を後に書く。辛口かつ建設的に。
